...
void nuskaitymas(vector<studentas> &vektorius, int &arivesti);
...
int main()
{
    ...
    nuskaitymas(vektorius, arivesti);
    skaiciavimas(vektorius);
    if (arivesti == 1)
    {
        isvedimas(vektorius);
    }
    else isvedimas2(vektorius);
}
...
void nuskaitymas(vector<studentas> &vektorius, int &arivesti)
{
    ...
    else
    {
        int Failas;
        string FailoPav;
        cout << "Is kurio failo norite nuskaityti duomenis? [1-3]" << endl;
